...
// api/link/generate.js
var PASSWORD = "nfs";
var DEFAULT_DURATION = "7\u5206\u949F";
var DEFAULT_DURATION_MS = 7 * 60 * 1e3;
...
// api/pickup/mail-code.js
var TARGET = "https://plus.keria.cc.cd/api/pickup/mail-code";
...
async function proxyFormRequest(request, target) {
  ...
  if (!contentType.includes("multipart/form-data") && !contentType.includes("application/x-www-form-urlencoded")) {
    return json2({ detail: "Unsupported content type" }, 415);
  }
  try {
    ...
    return json2({ detail: "Upstream request failed" }, 502);
  }
}
__name(proxyFormRequest, "proxyFormRequest");
function methodNotAllowed() {
  return json2({ detail: "Method Not Allowed" }, 405, { Allow: "POST" });
}
...
__name(withSafeHeaders, "withSafeHeaders");

// api/pickup/mail-keys.js
var TARGET2 = "https://plus.keria.cc.cd/api/pickup/mail-keys";
async function onRequestPost3({ request }) {
  return proxyFormRequest2(request, TARGET2);
}
...
async function proxyFormRequest2(request, target) {
  const contentType = request.headers.get("content-type") || "";
  if (!contentType.includes("multipart/form-data") && !contentType.includes("application/x-www-form-urlencoded")) {
    return json3({ detail: "Unsupported content type" }, 415);
  }
  try {
    const body = await request.formData();
    const response = await fetch(target, {
    ...
    return withSafeHeaders2(response);
  } catch {
    return json3({ detail: "Upstream request failed" }, 502);
  }
}
__name(proxyFormRequest2, "proxyFormRequest");
function methodNotAllowed2() {
  return json3({ detail: "Method Not Allowed" }, 405, { Allow: "POST" });
}
__name(methodNotAllowed2, "methodNotAllowed");
function json3(body, status, headers = {}) {
  return new Response(JSON.stringify(body), {
    status,
    headers: {
      "Content-Type": "application/json; charset=utf-8",
...
__name(json3, "json");
function withSafeHeaders2(response) {
  const headers = new Headers(response.headers);
  headers.set("Cache-Control", "no-store");
  headers.set("X-Content-Type-Options", "nosniff");
  headers.delete("set-cookie");
...
__name(withSafeHeaders2, "withSafeHeaders");
...
function onRequestGet2() {
  ...
  return new Response(JSON.stringify({
    ...
    beijing_time: new Date(now + 8 * 60 * 60 * 1e3).toISOString().replace("T", " ").replace(".000Z", "")
  }), {
    headers: {
      "Content-Type": "application/json; charset=utf-8",
      "Cache-Control": "no-store",
...
// ../.wrangler/tmp/pages-khCxaL/functionsRoutes-0.9809657619437381.mjs
var routes = [
  {
    routePath: "/api/link/generate",
    mountPath: "/api/link",
    method: "GET",
    middlewares: [],
...
// ../.wrangler/tmp/bundle-dMRVnb/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-dMRVnb/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.568341898514461.mjs.map
